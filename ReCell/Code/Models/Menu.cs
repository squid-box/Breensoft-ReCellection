namespace Recellection
{
	...
	public class Menu
	{
		...
		{
            this.explanation = this.insertLineBreaksForString(explanation);
			...
			switch (layout)
			{
				case Globals.MenuLayout.Prompt:
					this.CreatePrompt(icons);
					break;
				case Globals.MenuLayout.NineMatrix:
                    this.CreateNineMatrix(icons);
					break;
				case Globals.MenuLayout.FourMatrix:
                    this.CreateFourMatrix(icons);
					break;
			}
		}
		...
		{
			var regions = new List<GUIRegion>();
			foreach(MenuIcon mi in this.icons)
			{
        ...
        private void CreateFourMatrix(List<MenuIcon> icons)
        {
            ...
            this.menuPic = Recellection.textureMap.GetTexture(Globals.TextureTypes.TwoByTwo);
            ...
            for (int i = 0; i < icons.Count; i++)
            {
                ...
                {
                    icons[i].label = this.insertLineBreaksForString(icons[i].label);
        ...
        private void CreateNineMatrix(List<MenuIcon> icons)
        {
            ...
            int iconWidth = Recellection.viewPort.Width / 3;
            int iconHeight = Recellection.viewPort.Height / 3;

            this.menuPic = Recellection.textureMap.GetTexture(Globals.TextureTypes.ThreeByThree);
            for (int i = 0; i < icons.Count; i++)
            {
				MenuIcon mi = icons[i];
				...
				if (mi.label != null)
				{
                    mi.label = this.insertLineBreaksForString(mi.label);
        ...
        private void CreatePrompt(List<MenuIcon> icons)
        {
            ...
            this.menuPic = Recellection.textureMap.GetTexture(Globals.TextureTypes.PromptMenu);
        ...
        private void FreeStyle(int cols, int rows, List<MenuIcon> icons, Globals.TextureTypes menuTexture, bool scrollZone)
        {
            ...
            int iconWidth = Recellection.viewPort.Width / cols;
            int iconHeight = Recellection.viewPort.Height / rows;

            if (menuTexture != Globals.TextureTypes.NoTexture)
            {
                this.menuPic = Recellection.textureMap.GetTexture(menuTexture);
            }
            else
            {
