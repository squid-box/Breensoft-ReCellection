                ...
                foreach (Tile t in tileCollection)
                {
                    ...
                    if (b != null)
                    {
                        lock (b)
                        {
                            ...
                            Vector2 xyhpr1 = new Vector2((float)((b.position.Y - World.LookingAt.Y) * Globals.TILE_SIZE) +14, (float) Math.Round((b.position.Y - World.LookingAt.Y) * Globals.TILE_SIZE) +100);
                            Vector2 xyhpr2 = new Vector2((float)((b.position.Y - World.LookingAt.Y) * Globals.TILE_SIZE) + 114, (float)Math.Round((b.position.Y - World.LookingAt.Y) * Globals.TILE_SIZE) + 100);
                            Vector2 xyhpg2 = new Vector2((float)((b.position.Y - World.LookingAt.Y) * Globals.TILE_SIZE) + 14 + b.GetHealthPercentage(), (float)Math.Round((b.position.Y - World.LookingAt.Y) * Globals.TILE_SIZE) + 100);
